// Step 3: Mesh Smoothing (refinement)
...
public class Mesh_Refinement_Smooth : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // Step 3: Mesh refinement (mesh smoothing) using Remesher
        // Only change that was introduced from previous step 2 script.
        float EdgeLengthMultiplier = 5.5f;
        int _remeshPasses = 20;
        float smoothSpeedT = 1.0f;

        Remesher _remesh = new Remesher(meshResult);
        _remesh.PreventNormalFlips = true;
        _remesh.SetTargetEdgeLength(EdgeLengthMultiplier);
        _remesh.SmoothSpeedT = smoothSpeedT;
        _remesh.SetProjectionTarget(MeshProjectionTarget.Auto(meshResult));
